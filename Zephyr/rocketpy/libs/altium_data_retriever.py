...
Altium requirements portal API retriever for rocketpy simulation of Zephyr
...
#get session token (create or view in settings on altium requiremnts portal, reached via cogwheel in bottom left corner)
session_token = 'Bearer ' + getpass.getpass("Enter altium user key: ")

valispace = valispace.API(
    url='https://emea-tu-darmstadt-space-tech-ev-requirements.365.altium.com',
...
#GET COMPONENTS
components = valispace.get_component_list(project_name = "Zephyr")
...
sim_comps = {
    component["id"]: {
        key: component[key] for key in component.keys() if key in allowed_keys_components
    } for component in components
}

#GET VALIS
valis = valispace.get_vali_list(project_name="Zephyr")
...
sim_valis = [
    {
        key: vali[key] for key in vali.keys() if key in allowed_keys_valis
    } for vali in valis
]

#CLEANUP

#find rocket component
out_dict = get_rec_components([cmp["id"] for cmp in sim_comps.values() if cmp["name"] == "Rocket"][0])

#convert all positions to be absolute
...
for comp_name in extra_info:
    ...
    #get all specified valis
    comp_valis = {
        vali["shortname"]: clean_units(vali["value"], vali["unit"]) for vali in sim_valis
        if vali["parent"] == comp_id and vali["shortname"] in extra_info[comp_name]
    }
    #put valis in dictionary, insertion depending on existence
