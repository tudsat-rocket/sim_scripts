...
spec_folders = [
        {key: folder[key] for key in folder.keys() if key in allowed_keys_folders}
...
specifications = [
        {key: spec[key] for key in spec.keys() if key in allowed_keys_specifications}
    for spec in specifications]

req_groups = valispace.get_groups(project_id=42)
allowed_keys_groups = ["id", "name", "specification", "requirements", "description"]
req_groups = [
        {key: group[key] for key in group.keys() if key in allowed_keys_groups}
...
allowed_keys_requirements = ["title", "identifier", "specification", "id", "group",
                             ...
                             "rationale", "text",]
requirements = [
        {key: req[key] for key in req.keys() if key in allowed_keys_requirements}
...
req_tree_dict = {folder["name"]:
                {spec["name"]:
                    {"requirements": {req["identifier"]: req["id"] for req in requirements if req["id"] in spec["requirements"]},
                        "groups":
                            {group["name"]: {req["identifier"]: req["id"] for req in requirements if req["id"] in group["requirements"]}
